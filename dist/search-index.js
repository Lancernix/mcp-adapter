...
import { findServersInText, normalizeForSearch, removeMatchedServerTerms, tokenizeForSearch, } from "./search-utils.js";
const FUSE_OPTIONS = {
    includeScore: true,
    ignoreLocation: true,
    useTokenSearch: true,
...
// ---- rerank ----
function scoreFuseResult(doc, query, rawFuseScore, options) {
    const reasons = [];
    ...
    for (const token of queryTokens) {
        ...
        if (!options?.ignoreServerFields) {
            if (qualifiedText.includes(token))
                qualifiedHits++;
...
export class SearchIndex {
    ...
    buildIndex(servers, cachedServers) {
        ...
        if (this.documents.length > 0) {
            this.fuse = new Fuse(this.documents, FUSE_OPTIONS);
        }
        else {
            this.fuse = null;
        }
    ...
    search(query, targetServer, limit = 10) {
        ...
        if (targetServer) {
            ...
            if (first) {
                const removed = removeMatchedServerTerms(query, first.server, first.serverAliases);
                if (removed.trim()) {
                    searchQuery = removed;
                }
                else {
        ...
        if (scoped && functionQueryEmpty) {
            return this.browseDocs(candidates, limit, "query 仅命中服务名/别名，未提供功能关键词，返回该服务下工具浏览候选");
        }
        ...
        if (scoped || candidates.length < this.documents.length) {
            currentFuse = new Fuse(candidates, fuseOptions);
        }
     ...
     */
    tokenMatchFallback(query, candidates, limit, options) {
        ...
        const scored = candidates.map((doc) => {
            ...
            const onlyDescriptionHit = nameHits === 0 &&
                qualifiedHits === 0 &&
                serverHits === 0 &&
                aliasHits === 0 &&
                descHits > 0;
            ...
            const hasFunctionalHit = nameHits > 0 || qualifiedHits > 0 || descHits > 0;
            return {
                doc,
                score: Math.max(0, Math.min(100, Math.round(score))),
                reasons,
        ...
        return filtered.map(({ doc, score, reasons, hasFunctionalHit }) => ({
            ...
            inputSchema: doc.inputSchema,
            score,
            matchKind: scoped && !hasFunctionalHit ? "server_browse" : "token_fallback",
     ...
     */
    browseDocs(docs, limit = 10, reason = "已限定服务范围，但功能关键词未命中，返回服务内工具浏览候选") {
     ...
     */
    browseServer(serverName, limit = 10) {
