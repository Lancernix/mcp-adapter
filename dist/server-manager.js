...
import { computeServerHash, isServerCacheValid, loadMetadataCache, saveMetadataCache, } from "./cache-manager.js";
...
export class McpServerManager {
    ...
    metadataRefreshPromises = new Map();
     ...
     */
    async connectWithMeta(name, config, options) {
    ...
    async createConnection(name, config, options) {
        ...
        const connectTimeoutMs = config.connectTimeoutMs ?? options?.connectTimeoutMs ?? 60000;
        try {
            await withTimeout(client.connect(transport), connectTimeoutMs, `连接底层真实 MCP 服务 [${name}] 超时，超过 ${connectTimeoutMs}ms`);
            ...
            const cleanupTimeoutMs = config.closeTimeoutMs ?? options?.closeTimeoutMs ?? 10000;
            await withTimeout(client.close().catch(() => { }), cleanupTimeoutMs, `连接失败后关闭 client [${name}] 超时`).catch(() => { });
            await withTimeout(transport.close().catch(() => { }), cleanupTimeoutMs, `连接失败后关闭 transport [${name}] 超时`).catch(() => { });
            throw new Error(`连接底层真实 MCP 服务 [${name}] 失败: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    /**
    ...
    buildTransport(name, config) {
        if (config.type === "http" || config.type === "sse") {
            if (!config.url) {
                throw new Error(`[ServerManager] 服务 [${name}] 使用 HTTP/SSE 传输但未配置 url`);
            }
            const TransportClass = config.type === "sse"
                ? SSEClientTransport
        ...
        writeLog(`[ServerManager] 正在惰性唤醒真实的子进程 [${name}]: ${config.command} (${config.args?.length ?? 0} args)\n`);
     ...
     */
    async close(name, closeTimeoutMs = 10000, force = false) {
    ...
    async doClose(name, closeTimeoutMs, force) {
        const conn = this.connections.get(name);
        if (!conn)
            return;
        if (conn.inFlight > 0 && !force) {
            writeLog(`[ServerManager] 跳过关闭 [${name}]，当前仍有 ${conn.inFlight} 个请求执行中。\n`);
            return;
        }
        writeLog(`[ServerManager] 正在优雅销毁子进程 [${name}] 的 MCP 连接...\n`);
        conn.status = "closed";
        await withTimeout(conn.client.close().catch(() => { }), closeTimeoutMs, `关闭 MCP client [${name}] 超时`).catch(() => { });
        await withTimeout(conn.transport.close().catch(() => { }), closeTimeoutMs, `关闭 MCP transport [${name}] 超时`).catch(() => { });
     ...
     */
    async shutdownAll(closeTimeoutMs = 10000, force = true) {
        this.shuttingDown = true;
        // 先清理正在建连中但尚未完成的资源（防止僵尸进程泄漏）
        for (const [name, res] of this.pendingResources.entries()) {
            await withTimeout(res.client.close().catch(() => { }), closeTimeoutMs, `关闭 pending MCP client [${name}] 超时`).catch(() => { });
            await withTimeout(res.transport.close().catch(() => { }), closeTimeoutMs, `关闭 pending MCP transport [${name}] 超时`).catch(() => { });
            this.pendingResources.delete(name);
        }
        ...
        for (const [name, promise] of pendingConnects) {
            await withTimeout(promise.catch(() => undefined), closeTimeoutMs, `等待 pending connect [${name}] 结束超时`).catch(() => { });
        }
     ...
     */
    async refreshMetadataIfNeeded(name, config, options) {
     ...
     */
    async doRefreshMetadataIfNeeded(name, config, options) {
        ...
        const requestTimeoutMs = config.requestTimeoutMs ?? options?.requestTimeoutMs ?? 60000;
        conn.inFlight++;
        conn.lastUsedAt = Date.now();
        let shouldDropConnection = false;
        try {
            const response = await withTimeout(conn.client.listTools(), requestTimeoutMs, `获取 [${name}] 工具列表超时，超过 ${requestTimeoutMs}ms`);
            ...
            if (shouldDropConnection) {
                await this.close(name, config.closeTimeoutMs ?? options?.closeTimeoutMs ?? 10000, true).catch(() => { });
            }
            else if (options?.closeIfCreated && result.createdByThisCall) {
                await this.close(name, config.closeTimeoutMs ?? options?.closeTimeoutMs ?? 10000).catch(() => { });
            }
        }
    }
}
