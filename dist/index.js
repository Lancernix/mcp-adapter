...
import { z } from "zod";
import { getValidCachedServers, isServerCacheValid, loadMetadataCache, } from "./cache-manager.js";
import { getConfigPath, getMcpAdapterHome, loadConfig, resolveServerName, saveConfig, } from "./config-manager.js";
...
async function initialize() {
    writeLog(`[@lancernix/mcp-adapter] 正在从 ${getMcpAdapterHome()} 启动冷装载...\n`);
    try {
        config = loadConfig();
        setConfigRef(() => config);
    }
    catch (err) {
        writeLog(`[Error] 无法启动网关，config.json 加载失败: ${err instanceof Error ? err.message : String(err)}\n`);
        process.exit(1);
    }
...
async function shutdownAndExit(reason) {
    ...
    const closeTimeoutMs = configuredCloseTimeoutMs <= 0 ? 10000 : configuredCloseTimeoutMs;
...
// ---- Bootstrap ----
function getServersNeedingMetadataRefresh() {
...
function startBackgroundBootstrapIfNeeded() {
    if (bootstrapStarted)
        return;
    ...
    setTimeout(() => {
        bootstrapServersSequentially(missingServers).catch((err) => {
            ...
            writeLog(`[Bootstrap-Fatal] 后台 metadata 初始化异常中止: ${err instanceof Error ? err.message : String(err)}\n`);
        });
    }, 0);
}
async function bootstrapServersSequentially(servers) {
    ...
    writeLog(`[Bootstrap] 后台 metadata 初始化开始，共 ${servers.length} 个服务待刷新。\n`);
    for (const [name, srvConfig] of servers) {
        bootstrapStatus.current = name;
        try {
            writeLog(`[Bootstrap] 正在刷新 [${name}]...\n`);
            const refreshed = await serverManager.refreshMetadataIfNeeded(name, srvConfig, {
            ...
            if (refreshed && !validCachedServers[name]) {
                throw new Error(`刷新后未得到有效 metadata cache，server "${name}" 缓存写入可能失败`);
            }
    ...
    searchIndex.buildIndex(config.mcpServers, validCachedServers);
    const successCount = bootstrapStatus.completed - bootstrapStatus.errors.length;
    writeLog(`[Bootstrap] 后台 metadata 初始化完成。成功: ${successCount}，失败: ${bootstrapStatus.errors.length}\n`);
}
// ---- 辅助函数 ----
function findServersMentionedInQueryFromConfig(query, servers) {
    const names = [];
    ...
    for (const [serverName, srvConfig] of Object.entries(servers)) {
        ...
        for (const name of [serverName, ...(srvConfig.aliases || [])]) {
            ...
            const set = nameToServers.get(key) ?? new Set();
            set.add(serverName);
...
async function ensureServerMetadata(serverName) {
    ...
    try {
        ...
        writeLog(`[Metadata] 刷新 ${serverName} 工具元数据失败: ${err instanceof Error ? err.message : String(err)}\n`);
        return false;
    }
}
...
function stringifySchemaForSearch(schema) {
...
function buildSearchTitle(matches, targetServer) {
    const hasServerBrowse = matches.some((m) => m.matchKind === "server_browse");
    const hasTokenFallback = matches.some((m) => m.matchKind === "token_fallback");
    if (hasServerBrowse) {
        return targetServer
...
function disabledServerText(serverName) {
...
mcpServer.registerTool("search_tools", {
    description: "检索所有配置的 MCP 工具。推荐优先使用此工具。模糊匹配工具名、服务名、别名和描述正文。" +
    ...
    let inferredServer;
    let inferredServerMetadataOk;
    if (!targetServer) {
        const mentionedServers = findServersMentionedInQueryFromConfig(query, config.mcpServers);
    ...
    const effectiveLimit = Math.min(limit ?? config.settings?.toolSearchLimit ?? 10, 20);
    ...
    if (matches.length === 0 && targetServer) {
        ...
        const browseMatches = searchIndex.browseServer(targetServer, effectiveLimit);
        ...
        if (bootstrapStatus.running) {
            ...
            if (Object.keys(validCache).length === 0) {
                ...
                const availableServers = Object.entries(config.mcpServers)
                    ...
                    .map(([name, cfg]) => cfg.aliases?.length
    ...
    for (const [srv, tools] of grouped) {
        ...
        for (const match of tools) {
            ...
            if (match.matchKind !== "fuzzy") {
                const kindLabel = match.matchKind === "token_fallback"
                    ? "关键词兜底"
                    : "服务浏览兜底";
                replyText += `, ${kindLabel}`;
            }
...
// ---- describe_tool ----
mcpServer.registerTool("describe_tool", {
    description: "获取单个工具的完整定义和 inputSchema。用于已知具体工具名后确认参数，" +
...
mcpServer.registerTool("list_tools", {
    description: "列出指定 MCP Server 的全部工具名称。仅返回工具名，不返回描述和参数 Schema。" +
...
// ---- execute_tool ----
mcpServer.registerTool("execute_tool", {
    description: "执行底层的真实工具。如果目标子进程未运行，网关会执行 Lazy 惰性冷启动激活它，执行完毕原样返回原始结果。",
    inputSchema: ExecuteToolArgsSchema,
}, async ({ tool: toolInput, server: serverInput, arguments: toolArguments, }) => {
    const findResult = locateTool(toolInput, serverInput);
    let serverName;
    let originalName;
    let cacheHit = false;
    if (findResult && !("candidates" in findResult)) {
        ...
        if (!resolved) {
            throw new Error(`[mcp-adapter] 未能定位到工具 "${toolInput}"，请使用 search_tools 重新搜索。`);
        }
    ...
    try {
        const options = {
            connectTimeoutMs: serverConfig.connectTimeoutMs ?? config.settings?.connectTimeoutMs,
            requestTimeoutMs: serverConfig.requestTimeoutMs ?? config.settings?.requestTimeoutMs,
            closeTimeoutMs: serverConfig.closeTimeoutMs ?? config.settings?.closeTimeoutMs,
        };
        conn = await serverManager.connect(serverName, serverConfig, options);
    }
    catch (err) {
        throw new Error(`[mcp-adapter] 唤醒子进程 [${serverName}] 失败: ${err instanceof Error ? err.message : String(err)}`);
    }
    conn.inFlight++;
    const closeTimeoutMs = serverConfig.closeTimeoutMs ?? config.settings?.closeTimeoutMs ?? 10000;
    let shouldDropConnection = false;
    try {
        const requestTimeoutMs = serverConfig.requestTimeoutMs ??
            config.settings?.requestTimeoutMs ??
            60000;
        const rawResult = await withTimeout(conn.client.callTool({
            name: originalName,
            arguments: toolArguments,
        }), requestTimeoutMs, `执行工具 [${serverName}.${originalName}] 超时，超过 ${requestTimeoutMs}ms`);
        ...
        const errorType = classifyError(err instanceof Error ? err : new Error(String(err)));
        ...
        writeLog(`[HitRate] ${serverName}.${originalName} | cache=${cacheHit ? "hit" : "miss"} | ${Date.now() - startTime}ms\n`);
    }
});
// ---- 工具定位 ----
function parseQualifiedToolInput(input) {
    const text = input.trim();
    const lowerText = text.toLowerCase();
    const candidates = Object.keys(config.mcpServers).sort((a, b) => b.length - a.length);
...
function stripQualifiedPrefixForServer(toolInput, serverName) {
...
function resolveServerFromToolInput(toolInput, serverInput) {
    // A. 显式 server 参数优先
    if (serverInput) {
...
function locateTool(toolInput, serverInput) {
    const validServers = getValidCachedServers(config, loadMetadataCache());
    // A. 显式 server 参数优先
    if (serverInput) {
        ...
        if (srvName && validServers[srvName]) {
            ...
            const matched = validServers[srvName].tools.find((t) => t.name === toolName);
            if (matched) {
                return {
                    tool: {
                        server: srvName,
    ...
    if (toolInput.includes(".")) {
        ...
        if (parsed && validServers[parsed.server]) {
            ...
            const matched = validServers[srvName].tools.find((t) => t.name === toolPart);
            if (matched) {
                return {
                    tool: {
                        server: srvName,
...
// ---- CLI import ----
function importConfig(fromPath, dryRun = false) {
    ...
    try {
        ...
        for (const [name, serverConfig] of Object.entries(sourceServers)) {
            ...
            if (!parsedServer.success) {
                preview.push(`  - ${name} [跳过：配置格式非法] ${parsedServer.error.issues
                    .map((i) => `${i.path.join(".")}: ${i.message}`)
                    .join("; ")}`);
                continue;
            }
            const srv = parsedServer.data;
            const aliasSet = new Set([name]);
            ...
            preview.push(`  - ${name} (${imported.type ?? "stdio"})${imported.disabled ? " [已禁用]" : ""} aliases: [${imported.aliases?.join(", ")}]`);
            count++;
        }
        if (dryRun) {
        ...
        writeLog(`[Import-Error] 迁移失败: ${err instanceof Error ? err.message : String(err)}\n`);
        process.exit(1);
    }
}
// ---- 入口 ----
async function main() {
    ...
    if (args[0] === "import") {
        ...
        if (!fromPath) {
            writeLog("[Error] 未指定源配置文件。用法: mcp-adapter import --from ~/.claude.json [--dry-run]\n");
            process.exit(1);
        }
    ...
    writeLog("[mcp-adapter] 网关 Server 已经就绪，打通 Stdio Stdin/Stdout 通道。\n");
