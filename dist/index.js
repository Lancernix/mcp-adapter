...
import { loadMetadataCache } from "./cache-manager.js";
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "search_tools") {
        ...
        const targetServerInput = args?.server;
    ...
    if (name === "describe_tool") {
        const toolInput = args?.tool || "";
        const serverInput = args?.server;
...
function locateTool(toolInput, serverInput) {
    const latestCache = loadMetadataCache();
    if (!latestCache || !latestCache.servers)
        return null;
    let candidates = [];
 ...
 */
function importConfig(fromPath) {
