...
import type { ToolSearchDoc, ServerConfig, CachedTool } from "./types.js";
import { normalize, resolveServerName } from "./config-manager.js";
...
export class SearchIndex {
  ...
  buildIndex(
    ...
    for (const [serverName, serverEntry] of Object.entries(cachedServers)) {
      ...
      if (!config || config.disabled) continue; // 过滤被禁用的服务
    ...
    if (this.documents.length > 0) {
      ...
      this.fuse = new Fuse<ToolSearchDoc>(this.documents, {
        includeScore: true,
        ignoreLocation: true,
  ...
  search(
    ...
    if (candidates.length < this.documents.length) {
      currentFuse = new Fuse<ToolSearchDoc>(candidates, {
        includeScore: true,
        ignoreLocation: true,
        threshold: 0.45,
        keys: [
